...
import type { ClaudeSettings, HookGroup } from '../src/types.js';
...
test('installClaudeHook writes hook once and is idempotent', async () => {
  ...
  const count = hooks
    .flatMap((group: HookGroup) => group.hooks || [])
...
test('uninstallClaudeHook removes only promptbetter hook command', async () => {
  ...
  const raw = await fs.readFile(settingsPath, 'utf8');
  const parsed = JSON.parse(raw) as ClaudeSettings;
  const hooks = (parsed.hooks?.UserPromptSubmit ?? []).flatMap((group: HookGroup) => group.hooks || []);

  const promptbetterCount = hooks.filter(
    (h) => h.type === 'command' && h.command === CLAUDE_HOOK_COMMAND,
  ).length;
  const customCount = hooks.filter(
    (h) => h.type === 'command' && h.command === 'echo custom-hook',
  ).length;
